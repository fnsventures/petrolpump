-- Fix: same-day (and all Cash/UPI) settlements must prefill Night cash / Phone pay.
-- Bug: get_day_closing_breakdown returned only shift till as night_cash and omitted
-- suggested_* / same_day_* keys, so the UI never added settlement cash.
...
begin
  ...
  -- Per customer: payments first cover prior open (→ Collection); remainder that
  -- covers today's credit is same-day (excluded from Collection & Credit today).
  with cust as (
  ...
  -- ALL Cash/UPI settlements today must sit in Night cash / Phone pay (Collection on
  -- the left cancels the prior-day portion; same-day is excluded from Collection).
...
comment on function public.compute_day_closing_components(date) is
  'Day-closing totals. Same-day credit+settle excluded from collection/credit; all Cash/UPI settles prefill night cash / phone pay.';
...
begin
  ...
  v_suggested_cash := coalesce((v_components->>'suggested_night_cash')::numeric, v_shift_cash);
  v_suggested_phone := coalesce((v_components->>'suggested_phone_pay')::numeric, v_shift_phone);
  ...
  return jsonb_build_object(
    ...
    'settle_cash_total', coalesce((v_components->>'settle_cash_total')::numeric, 0),
    'settle_upi_total', coalesce((v_components->>'settle_upi_total')::numeric, 0),
    'suggested_night_cash', v_suggested_cash,
    'suggested_phone_pay', v_suggested_phone,
    'snapshot', v_use_snapshot,
    -- Unsaved: prefill with shift till + all Cash/UPI settlements today.
    'night_cash', case
...
comment on function public.get_day_closing_breakdown(date) is
  'Day closing components. Prefills night cash / phone pay with shift till + Cash/UPI settlements.';
