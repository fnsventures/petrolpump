...
- **CI:** `.github/workflows/deploy-pages.yml` — push to `staging` or `main` (or manual dispatch) builds `js/env.js` from environment secrets (`SUPABASE_URL`, `SUPABASE_ANON_KEY`) so prod and staging each use their own Supabase project, then deploys static assets to GitHub Pages.
