function normalizeSalaryMonth(t){if(!t)return"";const[a,n]=String(t).split("-");if(!a||!n)return"";const d=String(n).padStart(2,"0").slice(0,2);return`${a}-${d}-01`}function salaryMonthKey(t){const a=normalizeSalaryMonth(t);return a?a.slice(0,7):""}function suggestPaymentDate(t){const a=getLocalDateString(),n=salaryMonthKey(t),d=a.slice(0,7);if(!n||n===d)return a;if(n<d){const[l,m]=n.split("-").map(Number);return toLocalDateString(new Date(l,m,0))}return a}function isMissingSalaryMonthColumn(t){const a=String(t?.message||"");return/salary_month/i.test(a)||t?.code==="PGRST204"}function isMissingSalaryPaymentIdColumn(t){const a=String(t?.message||"");return/salary_payment_id/i.test(a)||t?.code==="PGRST204"}function getStaffSalaryMonthContext(t,a){const n=salaryStatusInfo(t.monthly_salary,a,t),{salary:d,pending:l}=computeSalaryBalance(t.monthly_salary,a,t);return{label:n.label,className:n.className,payable:d,pending:n.pending,advance:n.advance,paid:Number(a??0)}}function formatSalaryAmount(t){return t==null?"\u2014":formatCurrency(t)}function formatMonthLabel(t){if(!t)return"\u2014";const[a,n]=t.split("-").map(Number);return new Date(a,n-1,1).toLocaleDateString("en-IN",{month:"long",year:"numeric"})}const SALARY_SLIP_PRINT_CSS="css/salary-slip-print.css?v=2";function slipAssetUrl(t){return new URL(t,window.location.href).href}function getPfSettings(){const t=PumpSettings.getStation(),a=AppConfig.DEFAULT_STATION;return{establishmentCode:(t.pfEstablishmentCode||a.pfEstablishmentCode||"").trim()}}function roundMoney(t){return Math.round(Number(t)*100)/100}function computePfBreakdown(t,a){const n=roundMoney(Math.max(0,Number(t??0))),d=roundMoney(Math.max(0,Number(a?.pf_contribution??0))),l=n>0?Math.min(d,n):0,m=d,p=roundMoney(Math.max(0,n-l));return{gross:n,employeePf:l,employerPf:m,netSalary:p,fixedAmount:d}}function getPayPeriodLabel(t){if(!t)return"\u2014";const[a,n]=t.split("-").map(Number),d=new Date(a,n-1,1),l=new Date(a,n,0),m=p=>p.toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"});return`${m(d)} \u2013 ${m(l)}`}const AMOUNT_WORDS_ONES=["","One","Two","Three","Four","Five","Six","Seven","Eight","Nine","Ten","Eleven","Twelve","Thirteen","Fourteen","Fifteen","Sixteen","Seventeen","Eighteen","Nineteen"],AMOUNT_WORDS_TENS=["","","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"];function amountWordsUnder100(t){if(t<20)return AMOUNT_WORDS_ONES[t];const a=Math.floor(t/10),n=t%10;return`${AMOUNT_WORDS_TENS[a]}${n?` ${AMOUNT_WORDS_ONES[n]}`:""}`.trim()}function amountWordsUnder1000(t){if(t<100)return amountWordsUnder100(t);const a=Math.floor(t/100),n=t%100;return`${AMOUNT_WORDS_ONES[a]} Hundred${n?` ${amountWordsUnder100(n)}`:""}`.trim()}function amountWordsIndian(t){if(t===0)return"";if(t<1e3)return amountWordsUnder1000(t);if(t<1e5){const d=Math.floor(t/1e3),l=t%1e3;return`${amountWordsUnder1000(d)} Thousand${l?` ${amountWordsUnder1000(l)}`:""}`.trim()}if(t<1e7){const d=Math.floor(t/1e5),l=t%1e5;return`${amountWordsIndian(d)} Lakh${l?` ${amountWordsIndian(l)}`:""}`.trim()}const a=Math.floor(t/1e7),n=t%1e7;return`${amountWordsIndian(a)} Crore${n?` ${amountWordsIndian(n)}`:""}`.trim()}function amountInWordsINR(t){const a=roundMoney(Math.abs(Number(t)||0)),n=Math.floor(a),d=Math.round((a-n)*100);if(n===0&&d===0)return"Zero Rupees Only";let l=amountWordsIndian(n);return l=l?`${l} Rupees`:"Zero Rupees",d>0&&(l+=` and ${amountWordsIndian(d)} Paise`),`${l} Only`}function computeSalaryBalance(t,a,n){const d=Number(t??0),l=n?computePfBreakdown(d,n).netSalary:d,m=Number(a??0),p=Math.max(0,l-m),w=Math.max(0,m-l);return{salary:l,gross:d,totalPaid:m,pending:p,advance:w}}function salaryStatusInfo(t,a,n){const{salary:d,totalPaid:l,pending:m,advance:p}=computeSalaryBalance(t,a,n);return d<=0?{label:"No salary set",className:"salary-status--none",pending:m,advance:p}:p>.009?{label:"Advance paid",className:"salary-status--advance",pending:m,advance:p}:m<=.009?{label:"Fully paid",className:"salary-status--paid",pending:m,advance:p}:l>0?{label:"Partial",className:"salary-status--partial",pending:m,advance:p}:{label:"Unpaid",className:"salary-status--unpaid",pending:m,advance:p}}function paymentsForEmployee(t,a){return(t||[]).filter(n=>n.employee_id===a).sort((n,d)=>String(n.date).localeCompare(String(d.date)))}function salaryExpenseDescription(t,a){if(!t)return"Salary";const n=a!=null&&String(a).trim()!==""?String(a).trim():null;return`Salary: ${t.name}${n?` - ${n}`:""}`}function salaryDeleteButtonHtml(t,a,n){if(!n||!t?.id)return"";const d=a?.name||"staff";return AdminDelete.buttonHtml({selector:"salary-delete-btn",data:{paymentId:t.id,staffName:d,date:t.date,amount:t.amount},title:"Delete payment (admin)"})}function getStaffBalanceForMonth(t,a,n){const d=(n||[]).find(w=>w.id===t);if(!d)return null;const m=paidByStaffInRange(a).get(t)||0,p=computeSalaryBalance(d.monthly_salary,m,d);return{staff:d,paid:m,...p,status:salaryStatusInfo(d.monthly_salary,m,d)}}function paidByStaffInRange(t){const a=new Map;return(t||[]).forEach(n=>{const d=n.employee_id;a.set(d,(a.get(d)||0)+Number(n.amount??0))}),a}function buildSlipRef(t,a){const n=String(t||"").replace(/-/g,"").slice(0,8).toUpperCase();return`SAL-${a.replace("-","")}-${n}`}function buildSalarySlipHtml(t,a,n){const d=formatMonthLabel(n),l=getPayPeriodLabel(n),m=a.reduce((I,S)=>I+Number(S.amount??0),0),p=computePfBreakdown(t.monthly_salary,t),{pending:w,advance:V}=computeSalaryBalance(t.monthly_salary,m,t),L=PumpSettings.getStationGstin(),x=getPfSettings(),D=PumpSettings.getStationAddress(),k=PumpSettings.getStationContactLine(),R=buildSlipRef(t.id,n),T=formatDisplayDate(getLocalDateString()),N=t.pf_number?.trim()||"",_=t.pan_number?.trim()||"",W=t.phone_number?.trim()||"",K=t.address?.trim()||"",F=[];L&&F.push(`<span>GSTIN: ${escapeHtml(L)}</span>`),x.establishmentCode&&F.push(`<span>PF Est. code: ${escapeHtml(x.establishmentCode)}</span>`);const U=a.length?a.map((I,S)=>`
        <tr>
          <td>${S+1}</td>
          <td>${escapeHtml(formatDisplayDate(I.date))}</td>
          <td class="num">\u20B9 ${formatNumberPlain(I.amount)}</td>
          <td>${escapeHtml(I.note||"\u2014")}</td>
        </tr>`).join(""):'<tr><td colspan="4" style="text-align:center;color:#64748b">No salary disbursements recorded for this month</td></tr>',Y=V>.009?`<tr class="salary-slip-summary-balance"><td>Advance paid (over net salary)</td><td>\u20B9 ${formatNumberPlain(V)}</td></tr>`:w>.009?`<tr class="salary-slip-summary-balance"><td>Balance payable (net)</td><td>\u20B9 ${formatNumberPlain(w)}</td></tr>`:'<tr class="salary-slip-summary-paid"><td>Balance payable (net)</td><td>\u20B9 0.00 \u2014 Settled</td></tr>',z=p.employerPf>0?`
...
The linked expense entry will also be removed. This cannot be undone.`))return;const{error:o}=await window.supabaseClient.from("salary_payments").delete().eq("id",e.id);if(o){alert(AppError.getUserMessage(o)),AppError.report(o,{context:"deleteSalaryPayment",id:e.id});return}await ht(e,s),typeof AppCache<"u"&&AppCache&&CacheInvalidation.invalidate("operational"),await tt()}function st(e){!a||!e||e.dataset.salaryDeleteBound==="1"||(e.dataset.salaryDeleteBound="1",e.addEventListener("click",async s=>{const r=s.target.closest(".salary-delete-btn");if(!r)return;s.stopPropagation(),s.preventDefault();const i=r.getAttribute("data-payment-id"),o=H.find(f=>f.id===i)||await(async()=>{const f=rt();return(await mt(f)).find(u=>u.id===i)})();if(!o){alert("Payment not found. Refresh the page and try again.");return}const y=S.find(f=>f.id===o.employee_id);r.disabled=!0;try{await gt(o,y)}finally{r.disabled=!1}}))}function B(){return readMonthYearValue(k,R)||I}function Z(){return readMonthYearValue(L,x)||B()}function rt(){return readMonthYearValue(T,N)||B()}function bt(){writeMonthYearValue(T,N,B())}function Q(e){_&&(q=e,ot(e,B()),_.setAttribute("aria-hidden","false"),document.body.classList.add("modal-open"))}function O(){_&&(_.setAttribute("aria-hidden","true"),document.body.classList.remove("modal-open"),q=null,document.querySelectorAll(".salary-summary-table tbody tr.is-selected").forEach(e=>{e.classList.remove("is-selected")}))}function ot(e,s){const r=S.find(P=>P.id===e);if(!r)return;const o=paidByStaffInRange(H).get(e)||0,y=getStaffSalaryMonthContext(r,o),f=paymentsForEmployee(H,e),h=formatMonthLabel(s),u=document.getElementById("salary-detail-title"),$=document.getElementById("salary-detail-subtitle"),c=document.getElementById("salary-detail-stats"),g=document.getElementById("salary-detail-payments-body");u&&(u.textContent=r.name),$&&($.textContent=`${r.role_display||"Staff"} \xB7 ${h}`);const b=formatCurrency(y.pending),v=y.pending<=.009?"salary-detail-balance is-clear":"salary-detail-balance",E=computePfBreakdown(r.monthly_salary,r),C=r.pf_number?.trim();if(c&&(c.innerHTML=`
      ...
      `),g){const P=a?4:3;f.length?g.innerHTML=f.map(M=>`
          <tr>
            <td>${escapeHtml(formatDisplayDate(M.date))}</td>
            <td class="num">${formatCurrency(M.amount)}</td>
            <td>${escapeHtml(M.note??"\u2014")}</td>
            ${a?`<td class="table-actions">${salaryDeleteButtonHtml(M,r,!0)}</td>`:""}
          </tr>`).join(""):g.innerHTML=`<tr><td colspan="${P}" class="muted">No payments recorded for this month.</td></tr>`}U&&(U.disabled=!1,U.title="",U.onclick=async()=>{try{await runSalarySlipPrint(r,f,s)}catch(P){AppError.report(P,{context:"printSalarySlip"}),alert(AppError.getUserMessage(P)||"Could not open the print dialog.")}}),Y&&(Y.disabled=!1,Y.title="")}async function it(){try{S=await StaffEmployees.loadActiveEmployees(window.supabaseClient,{isAdmin:a,useCache:!0})}catch(e){AppError.report(e,{context:"loadStaffMembers"}),S=[]}return S}async function St(e){const s=new Map(S.map(i=>[i.id,i])),r=[...new Set((e||[]).filter(i=>i&&!s.has(i)))];if(!r.length)return s;try{(await StaffEmployees.resolveEmployeesByIds(window.supabaseClient,r)).forEach((o,y)=>s.set(y,o))}catch(i){AppError.report(i,{context:"staffMapForIds"})}return s}function vt(e,s=!0){if(!e)return;const r=e.value;e.innerHTML=s?'<option value="">Select staff</option>':"",S.forEach(i=>{const o=document.createElement("option");o.value=i.id,o.textContent=`${i.name}${i.role_display?` (${i.role_display})`:""}`,e.appendChild(o)}),r&&S.some(i=>i.id===r)&&(e.value=r)}async function Et(e,s){const{data:r,error:i}=await window.supabaseClient.from("salary_payments").select("id, employee_id, date, amount, note, salary_month").gte("date",e).lte("date",s).order("date",{ascending:!1});if(i){if(isMissingSalaryMonthColumn(i)){const{data:o,error:y}=await window.supabaseClient.from("salary_payments").select("id, employee_id, date, amount, note").gte("date",e).lte("date",s).order("date",{ascending:!1});return y?(AppError.report(y,{context:"loadPaymentsInRange"}),[]):o??[]}return AppError.report(i,{context:"loadPaymentsInRange"}),[]}return r??[]}async function j(e){const s=normalizeSalaryMonth(e);if(!s)return[];const{data:r,error:i}=await window.supabaseClient.from("salary_payments").select("id, employee_id, date, amount, note, salary_month").eq("salary_month",s).order("date",{ascending:!1});if(i){if(isMissingSalaryMonthColumn(i)){const[o,y]=e.split("-").map(Number),{start:f,end:h}=getMonthRange(o,y-1);return Et(f,h)}return AppError.report(i,{context:"loadPaymentsForSalaryMonth"}),[]}return r??[]}async function J(e){return e===B()&&H.length?H:j(e)}async function G(){if(!D)return;const e=m?.value,s=Z();if(!e||!s){D.classList.add("hidden");return}const r=S.find(u=>u.id===e);if(!r)return;const i=await J(s),o=getStaffBalanceForMonth(e,i,S);if(!o)return;const y=computePfBreakdown(r.monthly_salary,r),f=formatMonthLabel(s);let h;o.salary<=0?h="no salary configured":o.status.advance>.009?h=`advance ${formatCurrency(o.status.advance)} paid`:o.pending<=.009?h="fully paid":h=`${formatCurrency(o.pending)} remaining`,D.textContent=`${f}: net ${formatCurrency(y.netSalary)} \xB7 ${formatCurrency(o.paid)} paid \xB7 ${h}`,D.classList.remove("hidden")}async function lt(){const e=m?.value;if(!e){l&&(l.textContent="Select a staff member first.",l.classList.remove("hidden"));return}l?.classList.add("hidden");const s=Z(),r=await J(s),i=getStaffBalanceForMonth(e,r,S);if(!i||i.pending<=.009){w&&(w.value="");return}w&&(w.value=i.pending.toFixed(2))}async function dt(e){const s=document.getElementById("salary-summary-body"),r=document.getElementById("salary-kpi-payroll"),i=document.getElementById("salary-kpi-paid"),o=document.getElementById("salary-kpi-pending");if(!s)return;if(!S.length){s.innerHTML='<tr><td colspan="7" class="muted">Add staff in <a href="staff.html">HR \u2192 Staff</a> first.</td></tr>',r&&(r.textContent="\u2014"),i&&(i.textContent="\u2014"),o&&(o.textContent="\u2014");return}H=await j(e);const y=paidByStaffInRange(H);let f=0,h=0,u=0;S.forEach(c=>{const g=Number(c.monthly_salary??0),b=y.get(c.id)||0,{salary:v,pending:E}=computeSalaryBalance(g,b,c);f+=v,h+=b,u+=E}),r&&(r.textContent=formatCurrency(f)),i&&(i.textContent=formatCurrency(h)),o&&(o.textContent=formatCurrency(u));const $=document.getElementById("salary-kpi-note");$&&($.classList.add("hidden"),$.textContent=""),s.innerHTML=S.map(c=>{const g=y.get(c.id)||0,b=getStaffSalaryMonthContext(c,g),v=b.advance>.009?`<span class="muted">Advance ${formatCurrency(b.advance)}</span>`:formatCurrency(b.pending),E=escapeHtml(c.name),C=escapeHtml(c.role_display??"\u2014");return`
        ...
        `}).join(""),s.querySelectorAll("tr[data-staff-id]").forEach(c=>{const g=c.getAttribute("data-staff-id");c.addEventListener("click",b=>{b.target.closest("button")||(Q(g),s.querySelectorAll("tr.is-selected").forEach(v=>v.classList.remove("is-selected")),c.classList.add("is-selected"))}),c.addEventListener("keydown",b=>{(b.key==="Enter"||b.key===" ")&&(b.preventDefault(),Q(g))})}),s.querySelectorAll(".salary-view-btn").forEach(c=>{c.addEventListener("click",g=>{g.stopPropagation(),Q(c.getAttribute("data-staff-id"))})}),s.querySelectorAll(".salary-slip-btn").forEach(c=>{c.addEventListener("click",async g=>{if(g.stopPropagation(),c.disabled)return;const b=c.getAttribute("data-staff-id"),v=S.find(C=>C.id===b);if(!v)return;const E=paymentsForEmployee(H,b);try{await runSalarySlipPrint(v,E,e)}catch(C){AppError.report(C,{context:"printSalarySlipQuick"}),alert(AppError.getUserMessage(C)||"Could not open the print dialog.")}})}),s.querySelectorAll(".add-payment-btn").forEach(c=>{c.addEventListener("click",g=>{if(g.stopPropagation(),c.disabled)return;const b=c.getAttribute("data-staff-id");ct(b)})}),q&&ot(q,e),G()}function ct(e,s={}){const r=s.salaryMonth||B();m&&(m.value=e),writeMonthYearValue(L,x,r),p&&(p.value=suggestPaymentDate(r)),w&&(w.value=""),G().then(()=>lt()),document.querySelector('.settings-nav-item[data-section="record"]')?.click(),n?.scrollIntoView({behavior:"smooth"})}async function X(e){const s=document.getElementById("salary-payments-body");if(!s)return;const r=await j(e);if(!r.length){s.innerHTML=`<tr><td colspan="5" class="muted">No payments for ${escapeHtml(formatMonthLabel(e))} salary.</td></tr>`;return}const i=await St(r.map(o=>o.employee_id));s.innerHTML=r.map(o=>{const y=i.get(o.employee_id),f=escapeHtml(StaffEmployees.displayName(y)),h=o.employee_id,u=y?"":' disabled title="Staff record not found"';return`
          <tr>
        ...
        `}).join(""),s.querySelectorAll(".history-slip-btn").forEach(o=>{o.addEventListener("click",async()=>{const y=o.getAttribute("data-staff-id"),f=i.get(y);if(!f)return;const h=await mt(e),u=paymentsForEmployee(h,y);try{await runSalarySlipPrint(f,u,e)}catch($){AppError.report($,{context:"printHistorySlip"}),alert(AppError.getUserMessage($)||"Could not open the print dialog.")}})})}async function mt(e){return j(e)}async function tt(){await it(),vt(m);const e=B(),s=rt();e&&(await dt(e),await X(s))}n&&n.addEventListener("submit",async e=>{e.preventDefault();const s=n.querySelector('button[type="submit"]');s&&(s.disabled=!0,s.textContent="Saving\u2026"),d?.classList.add("hidden"),l?.classList.add("hidden");const r=m?.value,i=p?.value,o=Number(w?.value||0),y=document.getElementById("payment-note")?.value?.trim()||null,f=Z(),h=normalizeSalaryMonth(f||i?.slice(0,7)),u=()=>{s&&(s.disabled=!1,s.textContent="Save payment")};if(!r){u(),l?.classList.remove("hidden"),l&&(l.textContent="Select a staff member.");return}if(!i){u(),l?.classList.remove("hidden"),l&&(l.textContent="Payment date is required.");return}if(i>getLocalDateString()){u(),l?.classList.remove("hidden"),l&&(l.textContent="Payment date cannot be in the future.");return}if(o<=0){u(),l?.classList.remove("hidden"),l&&(l.textContent="Amount must be greater than 0.");return}if(!h){u(),l?.classList.remove("hidden"),l&&(l.textContent="Select the salary month this payment applies to.");return}const $=S.find(A=>A.id===r),c=await J(f),g=getStaffBalanceForMonth(r,c,S);if(g&&g.salary>0&&o>g.pending+.009){const A=roundMoney(o-g.pending),wt=g.pending<=.009?`Net salary for ${formatMonthLabel(f)} is already settled. Record ${formatCurrency(o)} as advance?`:`Amount exceeds remaining balance (${formatCurrency(g.pending)}). This will overpay by ${formatCurrency(A)}. Continue?`;if(!confirm(wt)){u();return}}const b={employee_id:r,date:i,amount:o,note:y,salary_month:h};t.session?.user?.id&&(b.created_by=t.session.user.id);let v=null,E=null;if({data:v,error:E}=await window.supabaseClient.from("salary_payments").insert(b).select("id").single(),E&&isMissingSalaryMonthColumn(E)){const A={employee_id:r,date:i,amount:o,note:y};t.session?.user?.id&&(A.created_by=t.session.user.id),{data:v,error:E}=await window.supabaseClient.from("salary_payments").insert(A).select("id").single()}if(E){u(),AppError.handle(E,{target:l});return}const C=salaryExpenseDescription($,y),P={date:i,category:"salary",description:C,amount:o};v?.id&&(P.salary_payment_id=v.id),t.session?.user?.id&&(P.created_by=t.session.user.id);let M=null;if({error:M}=await window.supabaseClient.from("expenses").insert(P),M&&isMissingSalaryPaymentIdColumn(M)&&(delete P.salary_payment_id,{error:M}=await window.supabaseClient.from("expenses").insert(P)),M){if(v?.id){const{error:A}=await window.supabaseClient.from("salary_payments").delete().eq("id",v.id);A&&AppError.report(A,{context:"rollbackSalaryPaymentAfterExpenseFail",paymentId:v.id})}u(),AppError.handle(M,{target:l});return}u();const $t=i,ft=f,et=r;finishRecordFormSave(n,{date:$t},{date:RECORD_DATE_KEYS.salaryPayment}),ft&&writeMonthYearValue(L,x,ft),d?.classList.remove("hidden"),await tt(),m&&et&&S.some(A=>A.id===et)&&(m.value=et),G(),typeof AppCache<"u"&&AppCache&&CacheInvalidation.invalidate("operational")}),m?.addEventListener("change",G),V?.addEventListener("click",lt);function yt(){const e=Z();p&&e&&(p.value=suggestPaymentDate(e)),G()}L?.addEventListener("change",yt),x?.addEventListener("change",yt);function pt(e,s,r){if(!e||!s)return;const i=async()=>{const o=readMonthYearValue(e,s);o&&await r(o)};e.addEventListener("change",i),s.addEventListener("change",i)}pt(k,R,async e=>{bt(),writeMonthYearValue(L,x,e),await dt(e),await X(e)}),pt(T,N,async e=>{await X(e)});const ut=document.getElementById("salary-download-csv");ut&&ut.addEventListener("click",async()=>{const e=B();if(!e)return;await it();const s=await j(e),r=paidByStaffInRange(s),i=["Name","Role","Net monthly (\u20B9)","Paid this month (\u20B9)","Remaining (\u20B9)","Status"],o=S.map(u=>{const $=r.get(u.id)||0,c=getStaffSalaryMonthContext(u,$),g=c.advance>.009?`Advance ${c.advance}`:String(c.pending);return[String(u.name??"").replace(/"/g,'""'),String(u.role_display??"").replace(/"/g,'""'),String(c.payable),String($),g,c.label]}),y=[i.join(","),...o.map(u=>u.map($=>`"${$}"`).join(","))].join(`
