...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  initDocumentTypeCategories();
  loadStaffList();
});

// ─── Section navigation ──────────────────────────────────────────────────────

const VALID_SECTIONS = ["station", "billing", "pumps", "users", "salaries", "attendance", "alerts", "expenses", "documents", "integrations", "access"];
...
// ─── Sortable category lists (expenses + vault document types) ───────────────

const CATEGORY_DRAG_HANDLE_SVG = `<svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true" focusable="false">
...
/**
 * Shared add / edit / delete / drag-reorder UI for slug+label category tables.
 */
function createSortableCategoryManager(opts) {
  let cache = [];
  const prefix = opts.prefix;
  const editSel = `.edit-${prefix}`;
  const saveSel = `.save-${prefix}`;
  const cancelSel = `.cancel-${prefix}`;
  const deleteSel = `.delete-${prefix}`;

  function dragHandleHtml(label) {
    return `<span class="expense-cat-drag-handle" role="button" tabindex="0" aria-label="Drag to reorder ${escapeHtml(label)}" title="Drag to reorder">${CATEGORY_DRAG_HANDLE_SVG}</span>`;
  }

  function rowHtml(row) {
    return `
    <tr data-category-id="${escapeHtml(row.id)}">
      <td class="expense-cat-drag-col">${dragHandleHtml(row.label)}</td>
      <td class="expense-cat-label">${escapeHtml(row.label)}</td>
      <td class="expense-cat-actions">
        <button type="button" class="button-secondary button-small edit-${prefix}" data-id="${escapeHtml(row.id)}" data-label="${escapeHtml(row.label)}">Edit</button>
        ${AdminDelete.buttonHtml({
          selector: `delete-${prefix}`,
  ...
  function render(rows) {
    ...
    if (!tbody) return;
    if (!rows.length) {
      tbody.innerHTML = `<tr><td colspan="3" class="muted">${escapeHtml(opts.emptyMessage)}</td></tr>`;
      return;
    }
  ...
  function nextSortOrder() {
    ...
    return Math.max(...cache.map((row) => row.sort_order)) + 1;
  }
  ...
  async function persistOrder(orderedIds) {
    ...
    const previousIds = cache.map((row) => row.id);
    if (orderedIds.length === previousIds.length && orderedIds.every((id, i) => id === previousIds[i])) {
      return;
    }

    const sortById = new Map(cache.map((row) => [row.id, row.sort_order]));
    const updates = [];
  ...
  function startEdit(btn) {
    ...
    labelCell.innerHTML = `<input type="text" class="expense-cat-label-input" maxlength="80" value="${escapeHtml(currentLabel)}" aria-label="${escapeHtml(opts.editAriaLabel)}" />`;
    actionsCell.innerHTML = `
    <button type="button" class="button-secondary button-small save-${prefix}" data-id="${escapeHtml(btn.dataset.id || "")}">Save</button>
    <button type="button" class="button-secondary button-small cancel-${prefix}">Cancel</button>`;
  ...
  async function saveLabel(btn) {
    ...
    const cached = cache.find((r) => r.id === id);
  ...
  function init() {
    ...
    tbody?.addEventListener("click", (e) => {
      ...
      const editBtn = t.closest(editSel);
      if (editBtn) {
        e.preventDefault();
        startEdit(editBtn);
        return;
      }
      const saveBtn = t.closest(saveSel);
      if (saveBtn) {
        e.preventDefault();
        void saveLabel(saveBtn);
        return;
      }
      const cancelBtn = t.closest(cancelSel);
      if (cancelBtn) {
        e.preventDefault();
        render(cache);
        return;
      }
      const deleteBtn = t.closest(deleteSel);
      if (deleteBtn) {
        e.preventDefault();
        void handleDelete(deleteBtn);
      }
    });

    const addForm = document.getElementById(opts.addFormId);
    const labelInput = document.getElementById(opts.labelInputId);
    const addError = document.getElementById(opts.addErrorId);
    const addSuccess = document.getElementById(opts.addSuccessId);
    ...
    if (addForm && labelInput) {
      addForm.addEventListener("submit", async (e) => {
        ...
        if (!label) {
          ...
          if (addError) {
            addError.textContent = `Enter a ${opts.noun} name.`;
        ...
        if (error) {
          if (error.code === "23505" && addError) {
            addError.textContent = `${opts.noun[0].toUpperCase()}${opts.noun.slice(1)} already exists.`;
            addError.classList.remove("hidden");
...
function initExpenseCategories() {
  createSortableCategoryManager({
    ...
    tbodyId: "settings-expense-categories",
    addFormId: "expense-category-add-form",
    labelInputId: "expense-category-label",
    addErrorId: "expense-category-add-error",
    addSuccessId: "expense-category-add-success",
    ...
    editAriaLabel: "Category name",
  }).init();
}

function initDocumentTypeCategories() {
  createSortableCategoryManager({
    table: "document_categories",
    tbodyId: "settings-document-categories",
    addFormId: "document-category-add-form",
    labelInputId: "document-category-label",
    addErrorId: "document-category-add-error",
    addSuccessId: "document-category-add-success",
    prefix: "document-category",
    emptyMessage: "No document types.",
    noun: "document type",
    usageTable: "invoice_documents",
    usageColumn: "category",
    usageNoun: "document(s)",
    deleteTitle: "Delete document type",
    editAriaLabel: "Document type name",
  }).init();
}
